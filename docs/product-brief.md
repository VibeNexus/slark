...
> **Review 3 拆分说明**：原 Sprint 4 把 Scribe + Facilitator 捆一起，工期严重低估。已拆分：本 Sprint 只做 Scribe 相关；Facilitator 延后到 **Sprint 7**（单独成 Sprint 以给足设计空间）。

**交付**：每个 Workflow Run 结束后自动沉淀 decisions / lessons 到项目知识池。
...
**战略价值**：Slark 成为**项目知识资产的管理后台**。用户团队的每一次协作产出自动沉淀。
...
**交付**：Team 成员能自己讨论出 Workflow，不依赖 Template。这是 Slark 相对 slock.ai / Cursor / ClawTeam 的**核心差异化能力**。

**范围**：
1. `Facilitator` System Agent（设计 + prompt 工程 + 输出 YAML 的稳定性验证）
2. "Create Workflow from Team Discussion" 入口
3. Workflow Design Session 对话流（Session thread 独立，不混入普通 channel）
4. Session 结束后 YAML draft 用户 Approve → 写入 workflows 表
5. 触发方式：Sprint 7 先**手动触发**（Q-8 决议建议），自动触发延后
6. 失败降级：Facilitator 卡住或用户 Reject → 提示"请使用 Sprint 2 Template 路径"

**战略价值**：Slark "Programmable AI Team OS" 的最独特机制落地。护城河从"可用"升级为"独一无二"。
...
- B-1 Git Worktree 多 Agent 隔离（见 `docs/clawteam-comparison.md`，🔴 高优先级但 MVP 不做）
...
> **Sprint 启动前规则**：每个 Sprint 启动前，负责人必须扫一遍本表，把 `Deadline <= 本 Sprint` 的 `Q-N` 全部拍板或显式延期，否则该 Sprint 不能动工。同时扫一遍 `docs/clawteam-comparison.md`（B-N 借鉴条目）、`docs/research/routa-analysis.md`、`docs/optimization-backlog.md`（🔴 优先级未排期条目），按需纳入当前 Sprint。
...
v0 阶段数据价值低，**升级时直接删除 `~/.slark/slark.db`**（不备份）。具体执行：
- Sprint 1 启动的第一步：`rm -f ~/.slark/slark.db`
- Seed 逻辑：启动时检测 db 不存在 → 按下方 Schema 总清单直接初始化空 db → 显示 Welcome 页引导 Create Project
- **不提供自动迁移路径**，老用户需要手动备份（用户层面负责）
...
| v1.0.1 | 2026-04-23 | v1.0 Review 决议落地：S-1 起止点明确为 Welcome → Send；S-2 / D-7 改表述为"同名 Agent 各归属不同 Project"；D-3 补兜底三件套逻辑；§11 加 wipe db 迁移策略；§9 拆分 Sprint 4（Scribe 单独，Facilitator 延后到 Sprint 7）；§10 加 Sprint 启动前扫描规则 | - |
